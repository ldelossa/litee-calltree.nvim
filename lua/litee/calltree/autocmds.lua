...
function M.jumpto_next_reference()
    ...
    if ctx.node.id ~= M.last_jumped_reference.node_id then
        return
    end
...
function M.highlight(set)
    ...
    -- do not reset the highlighting if "jumpto_next_reference" has been called
    -- for the same node under cursor
    if
        M.last_jumped_reference ~= nil and
        ctx.node.id == M.last_jumped_reference.node_id and
        M.last_jumped_reference.ref_idx ~= 1
    then
        return
    end
    ...
    if ctx.state["calltree"].direction == "to" then
        ...
        -- incoming calls can come from the same source file or different ones. 
        -- we only want to set highlights for ones in the current file.
        local name = vim.api.nvim_buf_get_name(ctx.state["calltree"].invoking_buf)
        if lib_path.strip_file_prefix(ctx.node.location.uri) == name then
            for i, ref in ipairs(ctx.node.references) do
                vim.api.nvim_buf_add_highlight(
                    ctx.state["calltree"].invoking_buf,
                    M.highlight_ns,
